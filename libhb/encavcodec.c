                    ...
                    hb_buffer_list_t *list )
{
        ...
        AV_PIX_FMT_VAAPI == pv->context->pix_fmt)
    {
       ...
       if( _oneShotEncodingLog01 ) {
           hb_log("encavcodec: Encode: send hw_frame (vaapi), format[frame 0x%X %s -> 0x%X %s, ctx 0x%X %s], same pts %d",
            frame.format, av_get_pix_fmt_name(frame.format),
            hw_frame->format, av_get_pix_fmt_name(hw_frame->format),
            pv->context->pix_fmt, av_get_pix_fmt_name(pv->context->pix_fmt),
            (hw_frame->pts == frame.pts));
           fflush(NULL);
       }
       // Encode
